// Дано N задач, каждая из которых имеет время начала и окончания.
// Нужно выбрать максимальное количество непересекающихся задач, чтобы выполнить их в одном рабочем дне.
// Сортируйте задачи по времени окончания и выбирайте те, которые начинаются после завершения предыдущей.
#include <iostream>
#include <vector>
#include <algorithm>
