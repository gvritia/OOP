// Напишите функцию, которая принимает односвязный список и разделяет его на два новых списка по следующему критерию:
// Первый список должен содержать узлы со значениями меньше заданного числа X.
// Второй список - узлы со значениями больше или равными X.
// X вводится с клавиатуры. Изначальный порядок элементов в каждом из новых списков должен быть сохранён. Создание новых узлов не требуется, используются только существующие
